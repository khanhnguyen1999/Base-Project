import { ADMIN_DESIGN_LIST } from "@core/constants/sidebar-menu";
...
const Member = (props: any) => {
  ...
  const ModuleFromName = (subPage: string, componentName: string, reducer: any) => {
    const lazyComponent = lazy(() =>
      import(
        `@modules/AdminManager/Design/pages/${subPage ? `${subPage}/${componentName}` : componentName}`
      ).then(async (component) => {
        if (reducer) {
          const m = await import(
            `@modules/AdminManager/Design/reducers/${reducer.resource}`
          );
  ...
  const routes = listComponent.routes.find((item: any) => {
    return item.path.includes("/design");
  });
  return (
    <Layout>
      <Sider theme="light">
        <AdminSidebar
          title={t("navmenu:admin:design")}
          menuList={ADMIN_DESIGN_LIST}
        />
      </Sider>
      <Layout>
        {
